...
def curvature(kpt0,band_num,b=0.00001,krel = True):
    ...
    #print(np.shape(vec))
    curv_z1 = np.log(np.vdot(vec[0],vec[1])*np.vdot(vec[1],vec[4])*np.vdot(vec[4],vec[2])*np.vdot(vec[2],vec[0]))
    curv_z2 = np.log(np.vdot(vec[3],vec[6])*np.vdot(vec[6],vec[7])*np.vdot(vec[7],vec[5])*np.vdot(vec[5],vec[3]))
    ...
    curv_y1 = np.log(np.vdot(vec[0],vec[3])*np.vdot(vec[3],vec[6])*np.vdot(vec[6],vec[1])*np.vdot(vec[1],vec[0]))
    curv_y2 = np.log(np.vdot(vec[2],vec[5])*np.vdot(vec[5],vec[7])*np.vdot(vec[7],vec[4])*np.vdot(vec[4],vec[2]))
    ...
    curv_x1 = np.log(np.vdot(vec[0],vec[2])*np.vdot(vec[2],vec[5])*np.vdot(vec[5],vec[3])*np.vdot(vec[3],vec[0]))
    curv_x2 = np.log(np.vdot(vec[1],vec[4])*np.vdot(vec[4],vec[7])*np.vdot(vec[7],vec[6])*np.vdot(vec[6],vec[1]))
...
def define(kpath,band_num, interval = 0.002,krel = True):
    ...
    for i in range(x-1):
        #bandn = np.where(np.argsort(w[i,:])==band_num-1)
        integral = integral + [np.vdot(v[i,:,np.argsort(w[i,:])[band_num-1]], v[i+1,:,np.argsort(w[i+1,:])[band_num-1]])-1]
#    overlap = overlap + [np.vdot(v[x-1,:,np.argsort(w[x-1,:])[band_num-1]],v[0,:,np.argsort(w[0,:])[band_num-1]])]
    #print(i)
...
def interpkpt(klist, num): # num must be a number or a array
    ...
    kpt = list([klist[0,:]])
    for i in range(m-1):
        kpt= kpt + list(np.transpose([np.linspace(klist[i,0],klist[i+1,0],n[i]),np.linspace(klist[i,1],klist[i+1,1],n[i]),np.linspace(klist[i,2],klist[i+1,2],n[i])]))[1:]
        #print(type(kpt))
    kpt = np.array(kpt,dtype='float')
    #print(np.shape(kpt))
    #print(kpt)
